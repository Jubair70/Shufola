...
def getDailyWeatherObservedList(request):
    ...
    query = "with t1 as (select case when station_id::int = 1 then 'Khulna' when station_id::int = 2 then 'Biratnagar' when station_id::int = 3 then 'Barisal' end station_name,substring(date_time from 1 for  20) as date_time, date_time::timestamp::date as date_info,coalesce(temperature,'') temperature,coalesce(dew_point_temperature,'') dew_point_temperature, coalesce(humidity,'') humidity,coalesce(solar_radiation,'') solar_radiation, coalesce(wind_speed,'') wind_speed, coalesce(wind_direction,'') wind_direction, coalesce(soil_moisture,'') soil_moisture, coalesce(rainfall,'') rainfall, coalesce(qfe,'') qfe,coalesce(qff,'') qff from weather_observed where station_id like '"+str(station_id)+"' AND date_time::timestamp BETWEEN '"+str(from_date)+" 00:00:00'::timestamp and '"+str(to_date)+" 23:59:59'::timestamp order by date_time::timestamp) select station_name, date_info,min(cast(temperature as double precision)) temp_min, max(cast(temperature as double precision)) temp_max, avg(cast(temperature as double precision)) temp_avg, min(cast(humidity as double precision)) humidity_min, max(cast(humidity as double precision)) humidity_max, avg(cast(humidity as double precision)) humidity_avg, sum(cast(solar_radiation as double precision)) sum_solar_radiation, sum(cast(rainfall as double precision)) sum_rainfall,max(cast(wind_speed as double precision)) max_wind_speed from t1 group by date_info,station_name"
...
def getWeatherObservedGraphData(request):
    ...
    to_date = request.POST.get('to_date')
    station_id = request.POST.get('station_name')
    temp_dict = __db_fetch_values_dict("with t1 as (select case when station_id::int = 1 then 'Khulna' when station_id::int = 2 then 'Biratnagar' when station_id::int = 3 then 'Barisal' end station_name,substring(date_time from 1 for  20) as date_time, date_time::timestamp::date as date_info,coalesce(temperature,'') temperature,coalesce(dew_point_temperature,'') dew_point_temperature, coalesce(humidity,'') humidity,coalesce(solar_radiation,'') solar_radiation, coalesce(wind_speed,'') wind_speed, coalesce(wind_direction,'') wind_direction, coalesce(soil_moisture,'') soil_moisture, coalesce(rainfall,'') rainfall, coalesce(qfe,'') qfe,coalesce(qff,'') qff from weather_observed where station_id like '"+str(station_id)+"' AND date_time::timestamp BETWEEN '"+str(from_date)+" 00:00:00'::timestamp and '"+str(to_date)+" 23:59:59'::timestamp order by date_time::timestamp) select station_name, date_info::text,min(cast(temperature as double precision)) temp_min, max(cast(temperature as double precision)) temp_max, avg(cast(temperature as double precision)) temp_avg,min(cast(humidity as double precision)) humidity_min, max(cast(humidity as double precision)) humidity_max, avg(cast(humidity as double precision)) humidity_avg,sum(cast(solar_radiation as double precision)) sum_solar_radiation,sum(cast(rainfall as double precision)) sum_rainfall from t1 group by date_info,station_name")
...
def get_sms_map(request):
    ...
    #dist_list = []
    dist_list = __db_fetch_values_dict("with t3 as( with t2 as (with t1 as (select farmer_id::text, crop_id::text, variety_id::text,season_id::text,stage_id::text,'weather' as sms_type from weather_sms_rule_queue where org_id::int = any('{" + str(org_list).strip('[]') + " }') and weather_sms_rule_queue.status = 'Sent' and weather_sms_rule_queue.content_type = 'text' and "+sub_query_date+" union select farmer_id::text,crop_id::text, variety_id::text,season_id::text, stage_id::text,'management' as sms_type from management_sms_que where organization_id::int = any('{" + str(org_list).strip('[]') + " }') and management_sms_que.status = 'Sent' and management_sms_que.content_type = 'text' and "+sub_query_date+" union (select promotional_sms.farmer_id::text,crop_id::text, variety_id::text,season_id::text, '100' as stage_id,'promotional' as sms_type from promotional_sms, sms_que where sms_que.sms_source = 'promotional_sms' and sms_que.status = 'Sent' and  promotional_sms.content_type = 'text' and promotional_sms."+sub_query_date+" and organization_id in ("+str(org_list_text).strip('[]')+") and sms_que.alertlog_id = promotional_sms.id )) select *, (select name from vwdistrict where id = (select district_id from farmer where farmer.id::text = t1.farmer_id)) district_name, (select district_id from farmer where farmer.id::text = t1.farmer_id) district_id, (select zone_id from farmer where farmer.id::text = t1.farmer_id) zone_id,(select country_id from farmer where farmer.id::text = t1.farmer_id) country_id,(select upazila_id from farmer where farmer.id::text = t1.farmer_id) upazila_id from t1 ) select * from t2 where t2.district_id::text LIKE '"+str(_district)+"' and t2.zone_id::text LIKE '"+str(_division)+"' and t2.upazila_id::text LIKE '"+str(_upazilla)+"' and t2.country_id::text LIKE '"+str(_country)+"' and t2.crop_id::text LIKE '"+str(_crop)+"' and t2.variety_id::text LIKE '"+str(_variety_crop)+"' and t2.season_id::text LIKE '"+str(_season)+"' and t2.sms_type::text LIKE '"+str(_advisory_type)+"') select t3.district_name, count(t3.district_name) as total_no_of_farmer from t3 group by t3.district_name")
...
def get_sms_bar(request):
    ...
    month_list = __db_fetch_values_dict("with t3 as( with t2 as (with t1 as (select farmer_id::text, crop_id::text, variety_id::text,season_id::text,stage_id::text,'weather' as sms_type, date_part('year',schedule_time::timestamp)::text as sent_year, to_char(to_timestamp (date_part('month', schedule_time::timestamp)::text, 'MM'), 'Month') as sent_month from weather_sms_rule_queue where org_id::int = any('{" + str(org_list).strip('[]') + " }') and weather_sms_rule_queue.status = 'Sent' and weather_sms_rule_queue.content_type = 'text' and " + sub_query_date + " union select farmer_id::text,crop_id::text, variety_id::text,season_id::text, stage_id::text,'management' as sms_type, date_part('year',schedule_time::timestamp)::text as sent_year, to_char(to_timestamp (date_part('month', schedule_time::timestamp)::text, 'MM'), 'Month') as sent_month from management_sms_que where organization_id::int = any('{" + str(org_list).strip('[]') + " }') and management_sms_que.status = 'Sent' and management_sms_que.content_type = 'text' and " + sub_query_date + " union (select promotional_sms.farmer_id::text,crop_id::text, variety_id::text,season_id::text, '100' as stage_id,'promotional' as sms_type, date_part('year',promotional_sms.schedule_time::timestamp)::text as sent_year, to_char(to_timestamp (date_part('month', promotional_sms.schedule_time::timestamp)::text, 'MM'), 'Month') as sent_month from promotional_sms, sms_que where sms_que.sms_source = 'promotional_sms' and sms_que.status = 'Sent' and  promotional_sms.content_type = 'text' and promotional_sms." + sub_query_date + " and organization_id in ("+str(org_list_text).strip('[]')+") and sms_que.alertlog_id = promotional_sms.id )) select *, (select name from vwdistrict where id = (select district_id from farmer where farmer.id::text = t1.farmer_id)) district_name, (select district_id from farmer where farmer.id::text = t1.farmer_id) district_id, (select zone_id from farmer where farmer.id::text = t1.farmer_id) zone_id,(select country_id from farmer where farmer.id::text = t1.farmer_id) country_id,(select upazila_id from farmer where farmer.id::text = t1.farmer_id) upazila_id from t1 ) select * from t2 where t2.district_id::text LIKE '" + str(_district) + "' and t2.zone_id::text LIKE '" + str(_division) + "' and t2.upazila_id::text LIKE '" + str(_upazilla) + "' and t2.country_id::text LIKE '" + str(_country) + "' and t2.crop_id::text LIKE '" + str(_crop) + "' and t2.variety_id::text LIKE '" + str(_variety_crop) + "' and t2.season_id::text LIKE '" + str(_season) + "' and t2.sms_type::text LIKE '" + str(_advisory_type) + "') select t3.sent_month, t3.sent_year, count(t3.sent_month) as total_no_of_sms_month from t3 group by t3.sent_year,t3.sent_month")
    ...
    year_list = __db_fetch_values_dict("with t3 as( with t2 as (with t1 as (select farmer_id::text, crop_id::text, variety_id::text,season_id::text,stage_id::text,'weather' as sms_type, date_part('year',schedule_time::timestamp)::text as sent_year from weather_sms_rule_queue where org_id::int = any('{" + str(org_list).strip('[]') + " }') and weather_sms_rule_queue.status = 'Sent' and weather_sms_rule_queue.content_type = 'text' and " + sub_query_date + " union select farmer_id::text,crop_id::text, variety_id::text,season_id::text, stage_id::text,'management' as sms_type, date_part('year',schedule_time::timestamp)::text as sent_year from management_sms_que where organization_id::int = any('{" + str(org_list).strip('[]') + " }') and management_sms_que.status = 'Sent' and management_sms_que.content_type = 'text' and " + sub_query_date + " union (select promotional_sms.farmer_id::text,crop_id::text, variety_id::text,season_id::text, '100' as stage_id,'promotional' as sms_type, date_part('year',promotional_sms.schedule_time::timestamp)::text as sent_year from promotional_sms, sms_que where sms_que.sms_source = 'promotional_sms' and sms_que.status = 'Sent' and  promotional_sms.content_type = 'text' and promotional_sms." + sub_query_date + " and organization_id in ("+str(org_list_text).strip('[]')+") and sms_que.alertlog_id = promotional_sms.id )) select *, (select name from vwdistrict where id = (select district_id from farmer where farmer.id::text = t1.farmer_id)) district_name, (select district_id from farmer where farmer.id::text = t1.farmer_id) district_id, (select zone_id from farmer where farmer.id::text = t1.farmer_id) zone_id,(select country_id from farmer where farmer.id::text = t1.farmer_id) country_id,(select upazila_id from farmer where farmer.id::text = t1.farmer_id) upazila_id from t1 ) select * from t2 where t2.district_id::text LIKE '" + str(_district) + "' and t2.zone_id::text LIKE '" + str(_division) + "' and t2.upazila_id::text LIKE '" + str(_upazilla) + "' and t2.country_id::text LIKE '" + str(_country) + "' and t2.crop_id::text LIKE '" + str(_crop) + "' and t2.variety_id::text LIKE '" + str(_variety_crop) + "' and t2.season_id::text LIKE '" + str(_season) + "' and t2.sms_type::text LIKE '" + str(_advisory_type) + "') select t3.sent_year, count(t3.sent_year) as total_no_of_farmer from t3 group by t3.sent_year")
...
def get_voice_sms_map(request):
    ...
    #dist_list = []
    dist_list = __db_fetch_values_dict("with t3 as( with t2 as (with t1 as (select farmer_id::text, crop_id::text, variety_id::text,season_id::text,stage_id::text,'weather' as sms_type from weather_sms_rule_queue where org_id::int = any('{" + str(org_list).strip('[]') + " }') and weather_sms_rule_queue.status = 'Sent' and weather_sms_rule_queue.content_type = 'audio' and "+sub_query_date+" union select farmer_id::text,crop_id::text, variety_id::text,season_id::text, stage_id::text,'management' as sms_type from management_sms_que where organization_id::int = any('{" + str(org_list).strip('[]') + " }') and management_sms_que.status = 'Sent' and management_sms_que.content_type = 'audio' and "+sub_query_date+" union (select promotional_sms.farmer_id::text,crop_id::text, variety_id::text,season_id::text, '100' as stage_id,'promotional' as sms_type from promotional_sms, sms_que where sms_que.sms_source = 'promotional_sms' and sms_que.status = 'Sent' and  promotional_sms.content_type = 'audio' and promotional_sms."+sub_query_date+" and organization_id in ("+str(org_list_text).strip('[]')+") and sms_que.alertlog_id = promotional_sms.id )) select *, (select name from vwdistrict where id = (select district_id from farmer where farmer.id::text = t1.farmer_id)) district_name, (select district_id from farmer where farmer.id::text = t1.farmer_id) district_id, (select zone_id from farmer where farmer.id::text = t1.farmer_id) zone_id,(select country_id from farmer where farmer.id::text = t1.farmer_id) country_id,(select upazila_id from farmer where farmer.id::text = t1.farmer_id) upazila_id from t1 ) select * from t2 where t2.district_id::text LIKE '"+str(_district)+"' and t2.zone_id::text LIKE '"+str(_division)+"' and t2.upazila_id::text LIKE '"+str(_upazilla)+"' and t2.country_id::text LIKE '"+str(_country)+"' and t2.crop_id::text LIKE '"+str(_crop)+"' and t2.variety_id::text LIKE '"+str(_variety_crop)+"' and t2.season_id::text LIKE '"+str(_season)+"' and t2.sms_type::text LIKE '"+str(_advisory_type)+"') select t3.district_name, count(t3.district_name) as total_no_of_farmer from t3 group by t3.district_name")
...
def get_voice_sms_bar(request):
    ...
    month_list = __db_fetch_values_dict("with t3 as( with t2 as (with t1 as (select farmer_id::text, crop_id::text, variety_id::text,season_id::text,stage_id::text,'weather' as sms_type, date_part('year',schedule_time::timestamp)::text as sent_year, to_char(to_timestamp (date_part('month', schedule_time::timestamp)::text, 'MM'), 'Month') as sent_month from weather_sms_rule_queue where org_id::int = any('{" + str(org_list).strip('[]') + " }') and weather_sms_rule_queue.status = 'Sent' and weather_sms_rule_queue.content_type = 'audio' and " + sub_query_date + " union select farmer_id::text,crop_id::text, variety_id::text,season_id::text, stage_id::text,'management' as sms_type, date_part('year',schedule_time::timestamp)::text as sent_year, to_char(to_timestamp (date_part('month', schedule_time::timestamp)::text, 'MM'), 'Month') as sent_month from management_sms_que where organization_id::int = any('{" + str(org_list).strip('[]') + " }') and management_sms_que.status = 'Sent' and management_sms_que.content_type = 'audio' and " + sub_query_date + " union (select promotional_sms.farmer_id::text,crop_id::text, variety_id::text,season_id::text, '100' as stage_id,'promotional' as sms_type, date_part('year',promotional_sms.schedule_time::timestamp)::text as sent_year, to_char(to_timestamp (date_part('month', promotional_sms.schedule_time::timestamp)::text, 'MM'), 'Month') as sent_month from promotional_sms, sms_que where sms_que.sms_source = 'promotional_sms' and sms_que.status = 'Sent' and  promotional_sms.content_type = 'audio' and promotional_sms." + sub_query_date + " and organization_id in ("+str(org_list_text).strip('[]')+") and sms_que.alertlog_id = promotional_sms.id )) select *, (select name from vwdistrict where id = (select district_id from farmer where farmer.id::text = t1.farmer_id)) district_name, (select district_id from farmer where farmer.id::text = t1.farmer_id) district_id, (select zone_id from farmer where farmer.id::text = t1.farmer_id) zone_id,(select country_id from farmer where farmer.id::text = t1.farmer_id) country_id,(select upazila_id from farmer where farmer.id::text = t1.farmer_id) upazila_id from t1 ) select * from t2 where t2.district_id::text LIKE '" + str(_district) + "' and t2.zone_id::text LIKE '" + str(_division) + "' and t2.upazila_id::text LIKE '" + str(_upazilla) + "' and t2.country_id::text LIKE '" + str(_country) + "' and t2.crop_id::text LIKE '" + str(_crop) + "' and t2.variety_id::text LIKE '" + str(_variety_crop) + "' and t2.season_id::text LIKE '" + str(_season) + "' and t2.sms_type::text LIKE '" + str(_advisory_type) + "') select t3.sent_month, t3.sent_year, count(t3.sent_month) as total_no_of_sms_month from t3 group by t3.sent_year,t3.sent_month")
    ...
    year_list = __db_fetch_values_dict("with t3 as( with t2 as (with t1 as (select farmer_id::text, crop_id::text, variety_id::text,season_id::text,stage_id::text,'weather' as sms_type, date_part('year',schedule_time::timestamp)::text as sent_year from weather_sms_rule_queue where org_id::int = any('{" + str(org_list).strip('[]') + " }') and weather_sms_rule_queue.status = 'Sent' and weather_sms_rule_queue.content_type = 'audio' and " + sub_query_date + " union select farmer_id::text,crop_id::text, variety_id::text,season_id::text, stage_id::text,'management' as sms_type, date_part('year',schedule_time::timestamp)::text as sent_year from management_sms_que where organization_id::int = any('{" + str(org_list).strip('[]') + " }') and management_sms_que.status = 'Sent' and management_sms_que.content_type = 'audio' and " + sub_query_date + " union (select promotional_sms.farmer_id::text,crop_id::text, variety_id::text,season_id::text, '100' as stage_id,'promotional' as sms_type, date_part('year',promotional_sms.schedule_time::timestamp)::text as sent_year from promotional_sms, sms_que where sms_que.sms_source = 'promotional_sms' and sms_que.status = 'Sent' and  promotional_sms.content_type = 'audio' and promotional_sms." + sub_query_date + " and organization_id in ("+str(org_list_text).strip('[]')+") and sms_que.alertlog_id = promotional_sms.id )) select *, (select name from vwdistrict where id = (select district_id from farmer where farmer.id::text = t1.farmer_id)) district_name, (select district_id from farmer where farmer.id::text = t1.farmer_id) district_id, (select zone_id from farmer where farmer.id::text = t1.farmer_id) zone_id,(select country_id from farmer where farmer.id::text = t1.farmer_id) country_id,(select upazila_id from farmer where farmer.id::text = t1.farmer_id) upazila_id from t1 ) select * from t2 where t2.district_id::text LIKE '" + str(_district) + "' and t2.zone_id::text LIKE '" + str(_division) + "' and t2.upazila_id::text LIKE '" + str(_upazilla) + "' and t2.country_id::text LIKE '" + str(_country) + "' and t2.crop_id::text LIKE '" + str(_crop) + "' and t2.variety_id::text LIKE '" + str(_variety_crop) + "' and t2.season_id::text LIKE '" + str(_season) + "' and t2.sms_type::text LIKE '" + str(_advisory_type) + "') select t3.sent_year, count(t3.sent_year) as total_no_of_farmer from t3 group by t3.sent_year")
