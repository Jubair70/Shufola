    ...
    try:
        ...
        if custom != 'default':
            return HttpResponseRedirect(
                '/usermodule/' + owner.username + '/projects-views/' + xform.id_string + '/?tab_selection=' + custom)
...
def custom_data_view(request, username, id_string):
    ...
    print("print submission_instance_query")
    ...
    if 'pngo' in request.GET:
        pngo = str(request.GET.get('pngo'))
    if 'district' in request.GET:
        district = str(request.GET.get('district'))
    if 'upazila' in request.GET:
        upazila = str(request.GET.get('upazila'))
    if 'union' in request.GET:
        union = str(request.GET.get('union'))
    if 'ward' in request.GET:
        ward = str(request.GET.get('ward'))
    if 'village' in request.GET:
        village = str(request.GET.get('village'))
    if 'collecion_from_date' in request.GET:
        collecion_from_date = str(request.GET.get('collecion_from_date'))
    if 'collecion_to_date' in request.GET:
        collecion_to_date = str(request.GET.get('collecion_to_date'))
    if 'submission_from_date' in request.GET:
        submission_from_date = str(request.GET.get('submission_from_date'))
    if 'submission_to_date' in request.GET:
        submission_to_date = str(request.GET.get('submission_to_date'))
    if 'sender' in request.GET:
        sender = str(request.GET.get('sender'))
    ...
    sub_query_status = ""
    sub_query_sender_pngo =''




    if pngo != '%' :
    ##    sub_query_pngo += " AND vwlogger_instance.user_id = any(SELECT user_id FROM public.usermodule_usermoduleprofile where organisation_name_id= " + str(pngo)+")"
        sub_query_sender_pngo += " AND vwlogger_instance.user_id = any(SELECT user_id FROM public.usermodule_usermoduleprofile where organisation_name_id= " + str(pngo)+")"
        if sender != '%':
##          sub_query_user += " AND vwlogger_instance.user_id = " + str(sender)
            sub_query_sender_pngo = ''
            sub_query_sender_pngo += " AND vwlogger_instance.user_id = " + str(sender)
    ...
    if collecion_from_date != '%' and collecion_to_date !='%' and collecion_from_date != '' and collecion_to_date !='':
        sub_collection_query_date_range += " AND vwlogger_instance.date_created BETWEEN '" + str(
            collecion_from_date) + "' AND '" + str(collecion_to_date) + "'"
    ...
    if xform.id == 333:
        ...
        db_test_column_query = "vwlogger_instance.id Uniqe_ID,  vwlogger_instance.json->>'respondent_name'Contact_Person, (select username from public.auth_user where id = vwlogger_instance.user_id) Sender_name ,to_char(vwlogger_instance.date_created,'DD Mon YYYY') Collection_Date, app_inst.status Data_Status"
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True
    elif xform.id == 402:
        db_test_column_query = "vwlogger_instance.id Uniqe_ID, vwlogger_instance.json->>'headmaster_name'Contact_Person, (select username from public.auth_user where id = vwlogger_instance.user_id) Sender_name ,to_char(vwlogger_instance.date_created,'DD Mon YYYY') Collection_Date, app_inst.status Data_Status"
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True
    ...
    submission_instance_query = "SELECT " + column_query + " FROM vwlogger_instance LEFT JOIN approval_instanceapproval app_inst ON app_inst.instance_id = vwlogger_instance.id WHERE xform_id = " + str(
        xform.id) + str(sub_query_sender_pngo) + str(sub_collection_query_date_range) +str(sub_submission_query_date_range)+ str(sub_query_status)+ \
                                str(sub_query_district)+str(sub_query_upazila)+str(sub_query_union)+\
                                str(sub_query_ward) + str(sub_query_village)+str(sub_query_action)
...
def pending_data_view(request, username, id_string):
    ...
    if submission_start and submission_end is not '%':
        if sub_query_user == '':
            sub_query_date_range += " WHERE p.date_created BETWEEN '" + str(submission_start) + "' AND '" + str(
                submission_end) + "'"
        else:
            sub_query_date_range += " AND p.date_created BETWEEN '" + str(submission_start) + "' AND '" + str(
                submission_end) + "'"
    if status is not '%' and status != 'custom':
    ...
    submission_instance_query = "SELECT * FROM   (SELECT " + column_query + " FROM logger_instance LEFT JOIN approval_instanceapproval app_inst ON app_inst.instance_id = logger_instance.id WHERE xform_id = " + str(
        xform.id) + " AND (app_inst.status='Pending' OR app_inst.status='New')) p" + str(sub_query_user) + str(
        sub_query_date_range) + str(sub_query_status)
    print
    submission_instance_query
    data_list = []
    col_names = []
    cursor = connection.cursor()
    try:
...
def instance(request, username, id_string):
    ...
    xform, is_owner, can_edit, can_view, can_submit, can_delete = get_form_permissions(id_string, username,
                                                                                       str(request.user))
