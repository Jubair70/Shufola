...
#    print tab_selection
    ...
    # Page filer Field Vaue (differrent type of list and value) creating for pass -------- Start--




    pngolistQuery = 'SELECT id, organization FROM public.usermodule_organizations where parent_organization_id is not null order by id asc;'
    pngoList = multipleValuedQuryExecution(pngolistQuery)
  #  userListQuery = 'SELECT id, username FROM public.auth_user where id >0 order by id asc'
 #   userList = multipleValuedQuryExecution(userListQuery)

    # ---------------------  District cascading  Feedback list (Jquery)  Start----------------------------

    disQuery = "SELECT id, district FROM public.vwdistrict"
    ...
    # Page filer differrent type of list and value creating for pass -------- End--
...
#MultiValues Query & Getting List
...
## List Making Generalization Function --------
...
"""
@Geo Location Hirarchy
"""
def getUpazilas_jq(request):
    selectedDistrict = request.POST.get('district')
    upazilaQuery = "SELECT id, upazila, districtid FROM public.vwupazila where districtid =" + selectedDistrict
...
def getUnions_jq(request):
    ...
    unionQuery = "SELECT id, union_name, upazilaid FROM public.vwunion where upazilaid = " + selectedUpazila
...
def getWards_jq(request):
    selectedUnion = request.POST.get('union')
    wardQuery = "SELECT id, ward, unionid FROM public.vwward where unionid = " + selectedUnion
    ward_List = makeTableList(wardQuery)
    jsonWardList = json.dumps({'ward_List': ward_List}, default=decimal_date_default)
    return HttpResponse(jsonWardList)


def getVillages_jq(request):
    selectedWard = request.POST.get('ward')
    villageQuery = "SELECT id, village, wardid FROM public.vwvillage where wardid = " + selectedWard
    village_List = makeTableList(villageQuery)
    jsonVillageList = json.dumps({'village_List': village_List}, default=decimal_date_default)
    return HttpResponse(jsonVillageList)

"""
@Get Sender id Accoridng to pngo
"""
def getSenderList(request):
    selectedPngo = request.POST.get('pngo')
    senderQuery = "SELECT user_id id , (select username FROM public.auth_user where id = user_id  ) user_name  FROM public.usermodule_usermoduleprofile  where organisation_name_id = "+selectedPngo ;


    sender_List = makeTableList(senderQuery)
    jsonSenderList = json.dumps({'sender_List': sender_List}, default=decimal_date_default)

    return HttpResponse(jsonSenderList)