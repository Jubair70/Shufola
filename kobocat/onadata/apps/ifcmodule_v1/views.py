...
def edit_crop_form(request, farmer_id, farmer_crop_id):
    info_query = "select farmer_id,crop_id,season_id,crop_variety_id,sowing_date::date,unit_id,land_size,zone_id,district_id,upazila_id,union_id from farmer_crop_info where id = " + str(
        farmer_crop_id)
    ...
    return render(request, 'ifcmodule/edit_crop_form.html',
                  {'farmer_crop_id': farmer_crop_id, 'data': data, 'farmer_id': farmer_id, 'district': district,
                   'upazilla': upazilla, 'union': union, 'variety': variety, 'zone': zone, 'season': season,
                   'crop': crop, 'unit': unit})
...
def update_crop_form(request):
    if request.POST:
        ...
        update_query = "UPDATE public.farmer_crop_info SET crop_id=" + str(crop) + ", season_id=" + str(
            season) + ", crop_variety_id=" + str(crop_variety) + ", sowing_date='" + str(
            sowing_date) + "', unit_id=" + str(unit) + ", land_size='" + str(
            land_size) + "', updated_at=now(), updated_by=" + str(user_id) + ", zone_id=" + str(
            zone_id) + ", district_id=" + str(district_id) + ", upazila_id=" + str(upazila_id) + ", union_id=" + str(
            union_id) + " WHERE id=" + str(farmer_crop_id) + ""
        print(update_query)
        __db_commit_query(update_query)
...
def getStage(request):
    ...
    query = "select id,stage_name from crop_stage where season_id::int =" + str(
        season_id) + " and crop_variety_id::int = " + str(var_id) + " and crop_id = " + str(crop_id)
    data = json.dumps(__db_fetch_values_dict(query))
    return HttpResponse(data)
...
def get_sms_table(request):
    ...
    q = "select id::text,farmer_name,mobile_number,sms_text,status,(select crop_name from vwcrop_variety where crop_id =management_sms_que.crop_id limit 1) crop_name,(select crop_name from vwcrop_variety where variety_id =management_sms_que.variety_id limit 1 ) variety_name from management_sms_que where crop_id::text like '" + str(
        crop) + "' " + date_query + " order by id asc"
...
def send_sms(request, id):
    q = "update management_sms_que set status = 'Sent',updated_at = NOW(),updated_by = " + str(
        request.user.id) + " where id = " + str(id)
    __db_commit_query(q)
...
def delete_weather_sms_form(request, sms_rule_id):
    delete_rules_details = "delete from weather_sms_rule_details where id in (select unnest(regexp_split_to_array(rules_relation, '[&||]'))::int from weather_sms_rule_relation where weather_sms_rule_id = " + str(
        sms_rule_id) + ")"
...
def getDivisions(request):
    ...
    query = "select distinct geo_zone_id id,division_name field_name from vwunion where geo_country_id = '" + str(
        obj) + "'"
...
def getDistricts(request):
    obj = request.POST.get('obj')
    query = "select distinct geo_district_id id,district_name field_name from vwunion where geo_zone_id = '" + str(
        obj) + "'"
    data = json.dumps(__db_fetch_values_dict(query), default=decimal_date_default)
    return HttpResponse(data)
...
def getUpazillas(request):
    obj = request.POST.get('obj')
    query = "select distinct geo_upazilla_id id,upazilla_name field_name from vwunion where geo_district_id = '" + str(
        obj) + "'"
    data = json.dumps(__db_fetch_values_dict(query), default=decimal_date_default)
    return HttpResponse(data)
...
def data_graph_view(request):
    ...
    return render(request, 'ifcmodule/data_graph_view.html',
                  {'country': country, 'crop': crop, 'organization': organization})
...
def getGraphData(request):
    ...
    country = request.POST.get('country')
    if crop is None:
        query_farmer = "select (select name from geo_zone where id = zone_id)division_name,count(*) farmer_count from farmer where organization_id::text like '" + str(
            organization) + "' and program_id::text like '" + str(program) + "' and country_id::text like '" + str(
            country) + "' group by zone_id "
        query_land = "select(select name from geo_zone where id = zone_id)division_name,sum(land_size::numeric) land_size from farmer_crop_info where unit_id = 2  and farmer_id = any(select id from farmer where organization_id::text like '" + str(
            organization) + "' and program_id::text like '" + str(program) + "' and country_id::text like '" + str(
            country) + "') group by zone_id"
    else:
        ...
        query_farmer = "select(select name from geo_zone where id = zone_id)division_name,count(*) farmer_count from farmer where organization_id::text like '" + str(
            organization) + "' and program_id::text like '" + str(program) + "' and country_id::text like '" + str(
            country) + "' and id = any(select farmer_id from farmer_crop_info where crop_id::text = any(string_to_array('" + str(
            crop) + "',','))) group by zone_id"
        query_land = "select(select name from geo_zone where id = zone_id)division_name,sum(land_size::numeric) land_size from farmer_crop_info where unit_id = 2 and crop_id::text = any(string_to_array('" + str(
            crop) + "',',')) and farmer_id = any(select id from farmer where organization_id::text like '" + str(
            organization) + "' and program_id::text like '" + str(program) + "' and country_id::text like '" + str(
            country) + "') group by zone_id"
    ...
    if crop_district == 'nan' and crop_upazilla == 'nan' and crop_union == 'nan':
        if check_input_validaity(crop_district, 'District', crop_zone):
            district_id = __db_fetch_single_value("select id from geo_district where name = '" + str(
                crop_district) + "' and geo_zone_id = " + str(crop_zone))
...
def process_bulk_data(file_url):
    df = pandas.read_excel('onadata/media/' + str(file_url),
                           ...
                           usecols="A:S",
                           converters={'Mobile No': str, 'Sowing Date': str, 'Land Size': str, 'Crop District': str,
                                       'Crop Upazilla': str, 'Crop Union': str})
                                                ...
                                                if check_input_validaity(row['Sowing Date'].strip(), 'Sowing Date'):
                                                    ...
                                                    if check_input_validaity(row['Land Unit'].strip(), 'Land Unit'):
                                                        ...
                                                        crop_geo_locations = check_crop_has_different_valid_geolocation(
                                                            row['Zone'].strip(), row['Crop District'],
                                                            row['Crop Upazilla'], row['Crop Union'])
                                                        # print crop_geo_locations
                                                        ...
                                                        try:
                                                            ...
                                                            if fc == 0:
                                                                try:
                                                                    ...
                                                                    if fid:
                                                                        ...
                                                                        if cc == 0:
                                                                            try:
                                                                                ...
                                                                                row[
                                                                                    'Problem Field'] = 'Database error (Crop)'
                                                                    ...
                                                                    row['Problem Field'] = 'Database error (Farmer)'
                                                                    invalid_df = invalid_df.append(row,
                                                                                                   ignore_index=True)
                                                            else:
                                                            ...
                                                            row[
                                                                'Problem Field'] = 'Database error (Farmer)'
                                                            invalid_df = invalid_df.append(row,
                                                                                           ignore_index=True)
                                                    else:
